...
function formatClearCookie(name: string): string {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const pending = verifyData<PendingPayload>(getCookie(cookieHeader, CH_PENDING));
  if (!pending) {
  ...
  res.setHeader("Set-Cookie", [
    formatClearCookie(CH_PENDING),
    formatSetCookie(CH_OK, token, 60 * 60),
  ]);
